...
// Lorentz Transform Grid, based on velocity ratio line.
	...
	ctx.strokeWidth= 1.5;
		...
		function __doSeg( a,b,c,d ) {
			...
			__doSeg( L*Math.cos(seg*Math.PI/72), L*Math.sin(seg*Math.PI/72), L*Math.cos((seg+1)*Math.PI/72),L*Math.sin((seg+1)*Math.PI/72) );
		}
		for( let seg = 0; seg < 144; seg++ ){ 
			doSegA2( seg );
		}
		if(1)
		{
...
*/
				...
				__doSeg( L*Math.cos(seg*Math.PI/18), L*Math.sin(seg*Math.PI/18), L*Math.cos((seg+1)*Math.PI/18),L*Math.sin((seg+1)*Math.PI/18) );
