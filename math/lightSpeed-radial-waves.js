...
function update( evt ) {
	...
	for( let n = 0; n <= nFrames; n++ ) {
		...
		for( let r = 0; r < 10; r++ ) {
			// each R is further back in time... 
			const now_ = now-r/10;

			if( !f.emits[r] ) {
				f.emits[r] = new Ring();
			}	
			const ring = f.emits[r];
			ring.emitted.length = 0;
			ring.bounces.length = 0
			const cx = (-runT/2-r/10)*V;
		for( let n = 0; n < 37; n++ ) {
			// dx dy is where the photon IS.
			let dx = cx + (now_+runT/2+2*r/10)*C*Math.cos( n*Math.PI/18 );
			let dy =      (now_+runT/2+2*r/10)*C*Math.sin( n*Math.PI/18);

			if( !prior_f || !prior_f.emits[r].bounces[n] ) {
				if( (dx-(f.Pc))*(dx-(f.Pc))/(lengthContract*lengthContract) + dy*dy > L*L ){
					const last = ring.emitted.length-1;
					const l = ring.emitted[last];
					const ot = ru.ObservedTime( now, {x:0,y:0,z:0}, {x:dx, y:dy,z:0}, { x:V,y:0,z:0},{x:0,y:0,z:0} );
					

					ring.bounces[n] = {x:dx, y:dy, ot: ot, at:now, tx:ot*V, ty:0, dx : (V*ot-dx), dy:(0-dy) };

				}
				ring.emitted.push( [500+xscale*dx //((-runT/2-r/10)*V+ C*(now_+2*r/10+runT/2)*Math.cos( n*Math.PI/18 ))
													, 500+xscale*dy]); //C*(now_+2*r/10+runT/2)*Math.sin( n*Math.PI/18) ] );
			} else {
				const lastBounce = ring.bounces[n] = prior_f.emits[r].bounces[n];
				ring.emitted.push( [500+xscale*(lastBounce.x + (lastBounce.dx)*(now-lastBounce.at)/(lastBounce.ot-lastBounce.at))
...
ctx.fillStyle = "white";
		...
		if( now.emits )
			for( let r = 0; r < now.emits.length; r++ ) {			
				ctx.beginPath();
				if( r == 0 ) 	ctx.strokeStyle = "green";
				else 	ctx.strokeStyle = "red";

				for( let n = 0; n < 37; n++  ) {
					if( !n ) ctx.moveTo( now.emits[r].emitted[n][0], now.emits[r].emitted[n][1] );
	...
	if(0)
		...
		ctx.strokeStyle = "blue";
			for( let f = 0; f < eventFrames.length; f++  ) {
					...
					if( !n ) ctx.moveTo( eventFrames[f].emits[r].emitted[n][0], eventFrames[f].emits[r].emitted[n][1] );
					else ctx.lineTo( eventFrames[f].emits[r].emitted[n][0], eventFrames[f].emits[r].emitted[n][1] );
			}
			ctx.stroke();
