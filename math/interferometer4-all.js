...
const xkeyFramesLength = [ 0,0,0,0];
const xkeyFrameTimes = [ -2,-1,0,1,2];
const xkeyFrameTimes2 = [ -2,-1,0,1,2];

const xkeyFrames_left = [
	{ x: 0, y: 0 },{ x: 0, y: 0 },{ x: 0, y: 0 },{ x: 0, y: 0 },{ x: 0, y: 0 }
]
const xkeyFramesLength_left = [ 0,0,0,0];
const xkeyFrameTimes_left = [ -2,-1,0,1,2];
...
addSpan( "Now", frames, -1, -runT/2, runT/1000, "Now" );
...
if(0) {
	...
	//sliders.spanTotalT.textContent = "Up(T): " + keyFrameTimes.map(a=>(lengthContract*a).toFixed(3)).join(" @ ") + "(" + (total/(keyFrameTimes[4]+2)).toFixed(3) + ")" ;
	...
	//vels1.push(tot);
	//sliders.spanTotal2T.textContent = "Left(T): " + keyFrameTimes_left.map((a,i)=>(keyFramesLength_left[i]/(lengthContract*a)).toFixed(3)).join(" @ ") ;
	sliders.spanTotal2T.textContent = "Left(2way): " + (vels1[1]+vels1[2]).toFixed(3) + " ("+(vels1[1]).toFixed(3)+"+"+(vels1[2]).toFixed(3)+")" + " Local:" +  (lengthContract*(vels1[1]+vels1[2])).toFixed(3);
...
class Photon {
 ...
 keyFramesLength = [ 0,0,0,0];
 keyFrameTimes = [ -2,-1,0,1,2];
 keyFrameTimes2 = [ -2,-1,0,1,2];

 keyFrames_left = [
	{ x: 0, y: 0 },{ x: 0, y: 0 },{ x: 0, y: 0 },{ x: 0, y: 0 },{ x: 0, y: 0 }
]
 keyFramesLength_left = [ 0,0,0,0];
 keyFrameTimes_left = [ -2,-1,0,1,2];
	...
	constructor() {
		...
		if( !l.first_photon ) l.first_photon = this;
		l.photons = this;
	...
	update() {
		...
		this.can_draw = true; 
		if( this.left ) {
			if( del > this.keyFrameTimes_left[4] ) { this.can_draw = false; return; }
			if( del > this.keyFrameTimes_left[3] ) seg = 3;
			else if( del > this.keyFrameTimes_left[2] ) seg = 2;
			else if( del > this.keyFrameTimes_left[1] ) seg = 1;
			else seg = 0;
			//console.log( "UPdating:", seg, del );
			const segDx = this.keyFrames_left[seg+1].x - this.keyFrames_left[seg].x;
			const segDy = this.keyFrames_left[seg+1].y - this.keyFrames_left[seg].y;
			let ofs = 0;
			for( let s = 1; s <= seg; s++ ) {
				ofs += this.keyFrameTimes_left[s]-this.keyFrameTimes_left[s-1];
			}
			const segL = Math.sqrt( segDx*segDx + segDy*segDy);
			const delT = del - this.keyFrameTimes_left[seg];
			const segNDx = segDx / segL;
			const segNDy = segDy / segL;
			this.seg = seg;
			this.x = this.keyFrames_left[seg].x + values.C * delT * segNDx;
			this.y = this.keyFrames_left[seg].y + values.C * delT * segNDy;

			
		} else {
			if( del > this.keyFrameTimes[4] ) { this.can_draw = false; return; }
			if( del > this.keyFrameTimes[3] ) seg = 3;
			else if( del > this.keyFrameTimes[2] ) seg = 2;
			else if( del > this.keyFrameTimes[1] ) seg = 1;
			else seg = 0;
			const segDx = this.keyFrames[seg+1].x - this.keyFrames[seg].x;
			const segDy = this.keyFrames[seg+1].y - this.keyFrames[seg].y;
			let ofs = 0;
			for( let s = 1; s <= seg; s++ ) {
				ofs += this.keyFrameTimes[s]-this.keyFrameTimes[s-1];
			}
			const segL = Math.sqrt( segDx*segDx + segDy*segDy);
			const delT = del - this.keyFrameTimes[seg];
			const segNDx = segDx / segL;
			const segNDy = segDy / segL;

			this.seg = seg;
			this.x = this.keyFrames[seg].x + values.C * delT * segNDx;
			this.y = this.keyFrames[seg].y + values.C * delT * segNDy;
	...
	ctx.beginPath();
	ctx.strokeStyle = "white";
	ctx.moveTo( 500 + values.Scale * this.keyFrames[0].x, 500 + values.Scale * this.keyFrames[0].y );
	ctx.lineTo( 500 + values.Scale * this.keyFrames[1].x, 500 + values.Scale * this.keyFrames[1].y );
	ctx.stroke();
	}
	...
	ctx.beginPath();
	ctx.strokeStyle = "red";
	ctx.moveTo(  500 + values.Scale * this.keyFrames[1].x, 500 + values.Scale * this.keyFrames[1].y );
	ctx.lineTo(  500 + values.Scale * this.keyFrames[2].x, 500 + values.Scale * this.keyFrames[2].y );
	ctx.stroke();
	...
	ctx.beginPath();
	ctx.strokeStyle = "blue";
	ctx.moveTo(  500 + values.Scale *this.keyFrames[2].x, 500 + values.Scale *this.keyFrames[2].y );
	ctx.lineTo(  500 + values.Scale *this.keyFrames[3].x, 500 + values.Scale *this.keyFrames[3].y );
	ctx.stroke();
	}
	...
	ctx.beginPath();
	ctx.strokeStyle = "green";
	ctx.moveTo(  500 + values.Scale *this.keyFrames[3].x, 500 + values.Scale *this.keyFrames[3].y );
	ctx.lineTo(  500 + values.Scale *this.keyFrames[4].x, 500 + values.Scale *this.keyFrames[4].y );
	ctx.stroke();
	}

	if( this.seg === 0 ) {
	ctx.beginPath();
	ctx.moveTo( 500 + values.Scale *this.keyFrames_left[0].x, 500 + values.Scale *this.keyFrames_left[0].y );
	ctx.lineTo( 500 + values.Scale *this.keyFrames_left[1].x, 500 + values.Scale *this.keyFrames_left[1].y );
	ctx.stroke();
	}
	...
	ctx.beginPath();
	ctx.strokeStyle = "red";
	ctx.moveTo( 500 + values.Scale * this.keyFrames_left[1].x, 500 + values.Scale *this.keyFrames_left[1].y );
	ctx.lineTo( 500 + values.Scale * this.keyFrames_left[2].x, 500 + values.Scale *this.keyFrames_left[2].y );
	ctx.stroke();
	}
	...
	ctx.beginPath();
	ctx.strokeStyle = "blue";
	ctx.moveTo( 500 + values.Scale * this.keyFrames_left[2].x, 500 + values.Scale *this.keyFrames_left[2].y );
	ctx.lineTo( 500 + values.Scale * this.keyFrames_left[3].x, 500 + values.Scale *this.keyFrames_left[3].y );
	ctx.stroke();
	}
	...
	updatePath() {
		...
		const T = this.keyFrameTimes[0] = this.t;
		this.keyFrames[0].x = offset.x + T * values.Velocity  * c;
		this.keyFrames[0].y = offset.y + T * values.Velocity  * s;
		const c0 = contract( 0, -1 );
		this.keyFrameTimes2[0] = this.keyFrameTimes[0];
	   
		this.keyFrameTimes[1] = realTimeToObservedTimeXYZ2( this.keyFrameTimes[0], values.Velocity, this.keyFrames[0].x, this.keyFrames[0].y, 0
				, values.Velocity, this.keyFrames[0].x + c0.x, this.keyFrames[0].y + c0.y, 0, c, s, c, s );
		//console.log( "T0_1:", this.keyFrameTimes);
		this.keyFrameTimes2[1] = this.keyFrameTimes[0] + (this.keyFrameTimes[1] - this.keyFrameTimes[0]) / gamma;
	   
		let delt = (this.keyFrameTimes[1] - this.keyFrameTimes[0])*values.C;
	   
		this.keyFrames[1].x = this.keyFrames[0].x + delt*Math.cos(angle_0);
		this.keyFrames[1].y = this.keyFrames[0].y - delt*Math.sin(angle_0);
	   
		const c1 = contract( 0, -1 );
		this.keyFrameTimes[2] = realTimeToObservedTimeXYZ2( this.keyFrameTimes[1], values.Velocity, this.keyFrames[1].x, this.keyFrames[1].y, 0
			, values.Velocity, this.keyFrames[1].x + c1.x, this.keyFrames[1].y + c1.y, 0, c, s, c, s );
		this.keyFrameTimes2[2] = this.keyFrameTimes[1] + (this.keyFrameTimes[2] - this.keyFrameTimes[1]) / gamma;
		delt = (this.keyFrameTimes[2] - this.keyFrameTimes[1])*values.C;
		this.keyFrames[2].x = this.keyFrames[1].x + delt * Math.cos(angle_0);
		this.keyFrames[2].y = this.keyFrames[1].y - delt * Math.sin(angle_0);
		
	   
		const c2 = contract( 0, 1 );
		this.keyFrameTimes[3] = realTimeToObservedTimeXYZ2( this.keyFrameTimes[2], values.Velocity, this.keyFrames[2].x, this.keyFrames[2].y, 0
			, values.Velocity, this.keyFrames[2].x + c2.x, this.keyFrames[2].y + c2.y, 0, c, s, c, s );
		this.keyFrameTimes2[3] = this.keyFrameTimes[2] + (this.keyFrameTimes[3] - this.keyFrameTimes[2]) / gamma;
		delt = (this.keyFrameTimes[3] - this.keyFrameTimes[2])*values.C;
		this.keyFrames[3].x = this.keyFrames[2].x + delt * Math.cos( angle_3 );
		this.keyFrames[3].y = this.keyFrames[2].y - delt * Math.sin( angle_3 );
	   
		const c3 = contract( 1, 0 );
		this.keyFrameTimes[4] = realTimeToObservedTimeXYZ2( this.keyFrameTimes[3], values.Velocity, this.keyFrames[3].x, this.keyFrames[3].y, 0
			, values.Velocity, this.keyFrames[3].x + c3.x, this.keyFrames[3].y + c3.y, 0, c, s, c, s );
		this.keyFrameTimes2[4] = this.keyFrameTimes[3] + (this.keyFrameTimes[4] - this.keyFrameTimes[3]) / gamma;
		delt = (this.keyFrameTimes[4] - this.keyFrameTimes[3])*values.C;
		this.keyFrames[4].x = this.keyFrames[3].x + delt * Math.cos( angle_4 );
		this.keyFrames[4].y = this.keyFrames[3].y - delt * Math.sin( angle_4 );
	   
	   
		this.keyFrames_left[0].x = this.keyFrames[0].x;
		this.keyFrames_left[0].y = this.keyFrames[0].y;
		this.keyFrameTimes_left[0] = this.keyFrameTimes[0];
	   
		this.keyFrames_left[1].x = this.keyFrames[1].x;
		this.keyFrames_left[1].y = this.keyFrames[1].y;
		this.keyFrameTimes_left[1] = this.keyFrameTimes[1];
	   
		const cl2 = contract( -1, 0 );
		this.keyFrameTimes_left[2] = realTimeToObservedTimeXYZ2( this.keyFrameTimes_left[1], values.Velocity, this.keyFrames_left[1].x, this.keyFrames_left[1].y, 0
			, values.Velocity, this.keyFrames_left[1].x + cl2.x, this.keyFrames_left[1].y + cl2.y, 0, c, s, c, s );
		delt = (this.keyFrameTimes_left[2] - this.keyFrameTimes_left[1])*values.C;
	   
		this.keyFrames_left[2].x = this.keyFrames_left[1].x + delt * Math.cos(angle_l2);
		this.keyFrames_left[2].y = this.keyFrames_left[1].y - delt * Math.sin(angle_l2);
	   
		this.keyFrameTimes_left[3] = this.keyFrameTimes[3];
		this.keyFrames_left[3].x = this.keyFrames[3].x;
		this.keyFrames_left[3].y = this.keyFrames[3].y;
	   
		this.keyFrameTimes_left[4] = this.keyFrameTimes[4];
		this.keyFrames_left[4].x = this.keyFrames[4].x;
		this.keyFrames_left[4].y = this.keyFrames[4].y;
	   
		//console.log( "T : ", -values.Velocity, keyFrameTimeskeyFrameTimes_left, keyFrameTimes, keyFrameTimes2 );
		for( let i = 0; i < 4; i++ ) {
			const x = this.keyFrames[1+i].x-this.keyFrames[0+i].x;
			const xx = x*x;
			const y = this.keyFrames[1+i].y-this.keyFrames[0+i].y;
			const yy = y*y;
			this.keyFramesLength[i] = Math.sqrt(xx+yy);
		...
		for( let i = 0; i < 4; i++ ) {
			const x = this.keyFrames_left[1+i].x-this.keyFrames_left[0+i].x;
			const xx = x*x;
			const y = this.keyFrames_left[1+i].y-this.keyFrames_left[0+i].y;
			const yy = y*y;
			this.keyFramesLength_left[i] = Math.sqrt(xx+yy);
			//console.log( "Length:", Math.sqrt( xx+yy));
		}
