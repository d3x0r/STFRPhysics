...
addSpan( "Left Arm Length", 1000, 1, 0, 10/1000, "ArmL" );
addSpan( "Top Arm Length", 1000, 1, 0, 10/1000, "ArmU" );
...
*/
	...
	const cl2 = contract( -values.ArmL, 0 );
	...
	const cl3 = contract( values.ArmL, 0 );
	keyFrameTimes_left[3] = realTimeToObservedTimeXYZ2( keyFrameTimes_left[2], values.Velocity, keyFrames_left[2].x, keyFrames_left[2].y, 0
		, values.Velocity, keyFrames_left[2].x + cl3.x, keyFrames_left[2].y + cl3.y, 0, c, s, c, s );
	//keyFrameTimes2[3] = keyFrameTimes[2] + (keyFrameTimes[3] - keyFrameTimes[2]) / gamma;
	delt = (keyFrameTimes_left[3] - keyFrameTimes_left[2])*values.C;
	keyFrames_left[3].x = keyFrames_left[2].x + delt * Math.cos( angle_4 );
	keyFrames_left[3].y = keyFrames_left[2].y - delt * Math.sin( angle_4 );
	...
	//vels1.push(tot);
	sliders.spanTotalT.textContent = "Up(2way): " + (vels1[1]+vels1[2]).toFixed(3) 
				+ " ("+(vels1[1]).toFixed(3)+"+"+(vels1[2]).toFixed(3)+")"
					+ " Local:" +  (Lgamma*(vels1[1]+vels1[2])).toFixed(3);
	...
	sliders.spanTotal2T.textContent = "Left(2way): " + (vels1[1]+vels1[2]).toFixed(3) 
					+ " ("+(vels1[1]).toFixed(3)+"+"+(vels1[2]).toFixed(3)+")" 
					+ " Local:" +  (Lgamma*(vels1[1]+vels1[2])).toFixed(3);
...
*/
	...
	drawRect( 500+ posx,0 , 500+posy,-values.Scale*(values.ArmU), 0 );

	drawRect( 500+ posx, + values.Scale*(1), 500+ posy,0, 1 );

	drawRect( 500+ posx, - values.Scale*(values.ArmL), 500+ posy,0, 1 );
