...
let L=0.5; // length of body (m)  (L/C = time of body (s))
...
const drawOpts = {
 ...
 drawObserverLocal : true,
 drawAsSeen: true,
 drawDistantFrame : false,
 drawInverse : true,
 drawGrid : true,
 drawXTGrid : false,
	forceSameEvent : false,
	lengthContract : true,
	showFreq : false,
}
...
function draw(  ) {
	...
	const timeBackSeeFront = RealTime( now, {x:V,y:0,z:0}, {x: (2*L-O)*lGam,y:0,z:0}, {x:V,y:0,z:0}, {x:0,y:0,z:0} )[0]*(tGam)
	const timeFrontSeeBack = RealTime( now, {x:V,y:0,z:0}, {x: (-2*L-O)*lGam,y:0,z:0}, {x:V,y:0,z:0}, {x:0,y:0,z:0} )[0]*(tGam)

	const timeSyncBackFront = RealTime( now + timeFrontSeeBack/tGam, {x:V,y:0,z:0}, {x: (2*L-O)*lGam,y:0,z:0}, {x:V,y:0,z:0}, {x:0,y:0,z:0} )[0]
	const timeSyncFrontBack = RealTime( now + timeBackSeeFront/tGam, {x:V,y:0,z:0}, {x: (-2*L-O)*lGam,y:0,z:0}, {x:V,y:0,z:0}, {x:0,y:0,z:0} )[0]
/*

A = 0 sends 0
B sets 0 when it hears 0, sends 0, 0
A = D, hears 0,0, sends D/2,0
B = E, hears D/2,0, sets (E+D/2), sends D/2,E+D/2
A = F, hears D+D/2,(E+D+D/2)/2
...
//-------------------------------
	...
	if( drawOpts.drawObserverLocal )
	{
		...
		drawClock( clockRadius*3, clockRadius*12, timeFrontSeeBack, "L SeeBck" );
		drawClock( clockRadius*3, -clockRadius*12, timeBackSeeFront, "L SeeFnt" );
		drawClock( clockRadius*6, clockRadius*12, timeSyncBackFront, "L SyncBck" );
		drawClock( clockRadius*6, -clockRadius*12, timeSyncFrontBack, "L SyncFnt" );


		//drawClock( clockRadius*3, -clockRadius*18, L*4*lGam/(timeBackSeeFront+timeFrontSeeBack), "L tot" );
		drawClock( clockRadius*0, -clockRadius*18, L*2*lGam, "Len" );
