...
class dlnQuat{
	...
	apply( v, del ) {
		...
		if( v instanceof v3 ) {
				...
				const q = this.r;
				const st = Math.sin( del * this.r.nL / 2 ); // normal * sin_theta
				const ct = Math.cos( del * this.r.nL / 2 );  //Math.cos( pl );   quaternion q.w  = (exp(lnQ)) [ *exp(lnQ.W=0) ]
	...
	getBasisT( del ) {
		...
		return { right  :new v3({ x : 1 - ( yy + zz ),  y :     ( wz + xy ), z :     ( xz - wy ) })
