...
// returns the principle rotation
lnQuat.prototype.prinicpal = function() {
	return new lnQuat( 0, mod( this.x, Math.PI*2 ), mod( this.y, Math.PI*2 ), mod( this.z, Math.PI*2 ) );
}
...
lnQuat.prototype.spin = function(th,axis,oct){
	...
	return finishRodrigues( q, oct-4, ax, ay, az, th );
}
...
lnQuat.prototype.sub = function( q, s ) {
	if( "undefined" === typeof s ) s = 1.0;
	this.x -= q.x;
	this.y -= q.y;
	this.z -= q.z;
	this.dirty = true;
...
lnQuat.prototype.sub2 = function( q, s ) {
	return new lnQuat(this.w, this.x, this.y, this.z).sub( q, s );
}
