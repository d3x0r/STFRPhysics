...
function finishRodrigues( q, oct, ac, as, ax, ay, az, th ) {
	...
	const AdB = q.nx*ax + q.ny*ay + q.nz*az;
		// q.x/sqrt(q.x*q.x+q.y*q.y+q.z*q.z) * this.x/sqrt(this.x*this.x+this.y*this.y) 
		// q.y/sqrt(q.x*q.x+q.y*q.y+q.z*q.z) * this.y/sqrt(this.x*this.x+this.y*this.y) 
		// q.z/sqrt(q.x*q.x+q.y*q.y+q.z*q.z) * this.z/sqrt(this.x*this.x+this.y*this.y) 

		// sqrt( q.x*q.x+q.y*q.y+q.z*q.z  * this.x*this.x+this.y*this.y) 
		/*

		cos(q)cos(this)-sin(q)sin(this)*AdB

		cos(q+this)/2+ cos(q-this)/2 - ( sin(q+this)/2 - sin(q-this)/2 ) *  AdB

		(  cos(q+this)+ cos(q-this) - ( cos(q+this) - cos(q-this) ) * ( AdB ) ) /2

		(  cos(q+this)+ cos(q-this) -  cos(q+this) * AdB + cos(q-this)  * AdB  ) /2 

		(  cos(q+this)-  cos(q+this) * AdB + cos(q-this) + cos(q-this)  * AdB  )/2

		(  cos(q+this) ( 1 - AdB ) + cos(q-this)  * ( 1 + AdB )  )/2
		
		ang1 = q+this
		ang2 = q-this
		(  cos(ang1) ( 1 - AdB ) + cos(ang2)  * ( 1 + AdB )  )/2
		*/

	const ang1 = q.nL+th
	const ang2 = q.nL-th
	const bad_cosCo2 = (  Math.cos(ang1) * ( 1 - AdB ) + Math.cos(ang2) * ( 1 + AdB )  )/2
	...
	let ang = acos( cosCo2 )*2; // this doesn't overflow; cos*cos*AdB/AdB  +sin*sin*AdB ends up always within 1.
