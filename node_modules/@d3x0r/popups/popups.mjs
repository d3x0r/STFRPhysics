...
class Popup {
	...
	constructor(caption_,parent,opts) {
		...
		const closeButton = !this.inFrame && ( opts?.enableClose || false );
...
*/
		push(group, toString_, opens) {
			...
			// "listItem"+suffix has no separator, while makeList emits "list-"+suffix
			// and parentList "list-container-"+suffix - so stylesheets written to the
			// -suffix convention never matched. Emit that form too; the original class
			// stays so anything already styled against it keeps working.
			if( this.opts.suffix ) newLi.classList.add( "listItem-" + this.opts.suffix );
			...
			if( this.opts.suffix ) newSubList.classList.add( "listSubList-" + this.opts.suffix );
			...
			if( this.opts.suffix ) treeLabel.classList.add( "listItemLabel-" + this.opts.suffix );
