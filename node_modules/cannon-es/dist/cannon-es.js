 ...
 *   log-quat modification {@link http://github.com/d3x0r/ d3x0r}
 */
class Mat3 {
  /**
...
class Vec3 {
  ...
  cross(vector, target, noLog) {
    ...
    if (target instanceof Vec3i) {
      const vl = vector.length();
      ...
      // use unit vector components


      const vx = vector.x / vl;
      const vy = vector.y / vl;
      const vz = vector.z / vl;
      const x = this.x / tl;
      const y = this.y / tl;
      const z = this.z / tl; // to get raw sin.
      ...
      if (target instanceof Vec3i) {
        ...
        if (tl2 > 0.000001) {
          const tla = Math.asin(Math.sqrt(tl2)) / tl2; // the actual angular torque is divided by the input length scalar too
          // that's why it's /l^2;  it's /l to normalize the vector to scale by the angle, but the angle is divided by the length of the vector

          target.x = tx * tla;
          target.y = ty * tla;
          target.z = tz * tla;
        } else {
          // near zero for sin(x)=x anyway...
          // actually if tl2 is very small, the resulting angle is very large
          target.x = tx * 1e5;
          target.y = ty * 1e5;
          target.z = tz * 1e5;
        }
      ...
      if (!noLog) console.log("normal cross");
      const vx = vector.x;
      const vy = vector.y;
      const vz = vector.z;
      const x = this.x;
  ...
  get i() {
    ...
    if (l > 0.00001) {
      // off by 1-e11 +3 for each 0
      const a = Math.asin(l) / l;
      return new Vec3i(this.x * a, this.y * a, this.z * a);
    }
...
class Vec3i extends Vec3 {
  ...
  cross(vector, target) {
    ...
    //const ti = this instanceof Vec3i;
    const vi = vector instanceof Vec3i;
    const vl_ = vector.length();
    ...
    const vs = vi ? Math.sin(vl_) : vl_; // sin

    const vscale = 1 / vl_;
    const l = this.length(); // angle
    ...
    const s = Math.sin(l); // sin
    // use unit vector components

    const vx = vector.x * vscale;
    const vy = vector.y * vscale;
    const vz = vector.z * vscale;
    const x = this.x / l;
    const y = this.y / l;
    const z = this.z / l; // to get raw sign.
    ...
    if (target instanceof Vec3i) {
      const l2 = target.lengthSquared();

      if (l2 > 0.00001) {
        // off by 1-e11 +3 for each 0
        const a = Math.asin(Math.sqrt(l2)) / l2; // the actual angular torque is divided by the input length scalar too
        // that's why it's /l^2;  it's /l to normalize the vector to scale by the angle, but the angle is divided by the length of the vector
      ...
      // this should be scaled by sin of angle and junk... but the angle is often 90 degrees proper.
      target.x *= vs * s;
      target.y *= vs * s;
      target.z *= vs * s;
    }
...
new Vec3();
/**
 * A Log Quaternion describes a rotation in 3D space. (No parameter, use setQuat)
 * 
 * 
 * @constructor
 * 
...
class Quaternion {
   ...
   */
  toString() {
    return `${this.nx * this.θ},${this.ny * this.θ},${this.nz * this.θ}`;
  }
  /**
   * Convert to an Array
   * @return [x, y, z]
  ...
  setFromAxisAngle(axis, angle) {
    ...
    this.nx = axis.x;
    this.ny = axis.y;
    this.nz = axis.z; //this.x = axis.x * angle;
    //this.y = axis.y * angle;
    //this.z = axis.z * angle;
  ...
  setFromVectors(u, v) {
    console.log("Set From Vectors is not completed?");
  ...
  */
  mult(q, target, dt, internal) {
    ...
    if (q.θ) {
      ...
      const xmy = (th - dt * q.θ) / 2; // X - Y  (x minus y)

      const xpy = (th + dt * q.θ) / 2; // X + Y  (x plus y )

      const cxmy = Math.cos(xmy);
      const cxpy = Math.cos(xpy);
      const cosCo2 = ((1 - AdotB) * cxmy + (1 + AdotB) * cxpy) / 2;
      const ang = Math.acos(cosCo2) * 2;
  ...
  conjugate(target) {
    ...
    target.nx = this.nx;
    target.ny = this.ny;
    target.nz = this.nz; //target.x = -this.x;
    //target.y = -this.y;
    //target.z = -this.z;
   ...
   */
  normalize() {
    // this could normalize the angle...
    return this;
  }
  ...
  pitch(th, target) {
    ...
    const cnx = c * q.nx;
    tmpQ.nx = cnx * q.nx + c1;
    tmpQ.ny = cnx * q.ny + s * q.nz;
    tmpQ.nz = cnx * q.nz - s * q.ny;
    return this.mult(tmpQ, target);
  }

  roll(th, target) {
    const q = this; // input angle...
    ...
    tmpQ.θ = th;
    const cnz = c * q.nz;
    tmpQ.nx = cnz * q.nx + s * q.ny;
    tmpQ.ny = cnz * q.ny - s * q.nx;
    tmpQ.nz = cnz * q.nz + c1;
    return this.mult(tmpQ, target);
  }
  ...
  yaw(th, target) {
    ...
    tmpQ.θ = th;
    const cny = c * q.ny;
    tmpQ.nx = cny * q.nx - s * q.nz;
    tmpQ.ny = cny * q.ny + c1;
    tmpQ.nz = cny * q.nz + s * q.nx; //console.log( "Rotate ", q.nx, q.ny, q.nz, ax, ay, az, th );
  ...
  setFromEuler(x, y, z, order) {
    ...
    if (order === 'XYZ') {
      this.set(x, 0, 0).yaw(y, this).roll(z, this);
    } else if (order === 'YXZ') {
      this.set(0, y, 0).pitch(x, this).roll(z, this);
    } else if (order === 'ZXY') {
      this.set(0, 0, z).pitch(x, this).yaw(y, this);
    } else if (order === 'ZYX') {
      this.set(0, 0, z).yaw(y, this).pitch(x, this);
    } else if (order === 'YZX') {
      this.set(0, y, 0).roll(z, this).pitch(x, this);
    } else
      /*if ( order === 'XZY' )*/
      {
        this.set(x, 0, 0).roll(z, this).yaw(y, this);
      }
  ...
  slerp(toQuat, t, target) {
    ...
    target.copy(toQuat); // remove the rotation of q from p...

    target.mult(this, target, -1); // which sets target as the initial P rotation.

    if (!target.θ) {
      target.nx = toQuat.nx;
      target.ny = toQuat.ny;
      target.nz = toQuat.nz;
      target.θ = toQuat.θ; //target.x = target.nx * target.θ;
      //target.y = target.ny * target.θ;
      //target.z = target.nz * target.θ;
    ...
    axisTemp.z = target.nz; //    let tmpA;
    //    if( !external ) // delta angle is from an internal source
    //       tmpA = this.vmult( axisTemp, 1 );
    //    else
    //      tmpA = axisTemp;
    ...
    tmpA.θ = target.θ;
    target.copy(this);
    return target.mult(tmpA, target, t);
    /*
    target.x = this.x * (1-t) + toQuat.x * t;
    target.y = this.y * (1-t) + toQuat.y * t;
    target.z = this.z * (1-t) + toQuat.z * t;
  ...
  integrate(angularVelocity, dt, angularFactor, target) {
    ...
    integrateTempAVQ.set(angularVelocity.x * angularFactor.x, angularVelocity.y * angularFactor.y, angularVelocity.z * angularFactor.z);
    return this.mult(integrateTempAVQ, target, dt);
  }
  /**
...
class Transform {
  ...
  static pointToLocalFrame(position, quaternion, worldPoint, result) {
    ...
    worldPoint.vsub(position, result); //quaternion.conjugate(tmpQuat)

    quaternion.vmult(result, result, -1);
    return result;
  }
  /**
   * pointToWorldFrame
...
class ConvexPolyhedron extends Shape {
  ...
  findSeparatingAxis(hullB, posA, quatA, posB, quatB, target, faceListA, faceListB) {
    ...
    for (let e0 = 0; e0 !== hullA.uniqueEdges.length; e0++) {
      ...
      for (let e1 = 0; e1 !== hullB.uniqueEdges.length; e1++) {
        ...
        worldEdge0.cross(worldEdge1, Cross, true);
  ...
  clipFaceAgainstHull(separatingNormal, posA, quatA, worldVertsB1, minDist, maxDist, result) {
    ...
    for (let i = 0; i < numVerticesA; i++) {
      ...
      WorldEdge0.cross(worldPlaneAnormal1, planeNormalWS1, true);
...
class Body extends EventTarget {
  ...
  constructor(options) {
    ...
    this.angularVelocity = new Vec3i();

    if (options.angularVelocity) {
      this.angularVelocity.copy(options.angularVelocity);
    }

    this.initAngularVelocity = new Vec3i();
    this.shapes = [];
    this.shapeOffsets = [];
  ...
  applyForce(force, relativePoint) {
    ...
    const totForce = force.lengthSquared(); // Compute produced rotational force

    const rotForce = Body_applyForce_rotForce;
    relativePoint.cross(force, rotForce);
    const rotForceX = rotForce.lengthSquared(); // remove the force applied to rotation so we don't duplicate force.

    force.scale(Math.sqrt(totForce - rotForceX) / Math.sqrt(totForce)); // Add linear force
  ...
  applyAccel(force) {
    ...
    if (this.sleepState === Body.SLEEPING) {
      this.wakeUp();
    } //force.scale( this.invMass, Body_applyAccel_Force ); 
    // Add linear force
  ...
  applyTorque(torque) {
    ...
    this.torque.vadd(torque, this.torque); //    this.angularVelocity.vadd(torque, this.angularVelocity);
  }
  /**
  ...
  applyImpulse(mass, impulse, relativePoint) {
    if (relativePoint === void 0) {
      relativePoint = new Vec3();
    }
    ...
    const velo = Body_applyImpulse_velo; //	const ;
    ...
    if (r.lengthSquared()) {
      ...
      n.x = -n.x;
      n.y = -n.y;
      n.z = -n.z; // how 'forward' this impulse is.

      const dot = r.dot(impulse);
      velo.copy(impulse);
      velo.scale(dot, velo);
    } // Add linear impulse
    ...
    //console.log( "Adding velocity (was):", this.angularVelocity );

    this.angularVelocity.vadd(rotVelo, this.angularVelocity); //console.log( "Adding velocity  (is):", this.angularVelocity );
  }
  /**
  ...
  applyLocalImpulse(mass, localImpulse, localPoint) {
    if (localPoint === void 0) {
      localPoint = new Vec3();
    }
    ...
    this.vectorToWorldFrame(localPoint, relativePointWorld);
    this.applyImpulse(mass, worldImpulse, relativePointWorld);
  }
  /**
  ...
  getVelocityAtWorldPoint(worldPoint, result) {
    ...
    this.angularVelocity.cross(r, result); //this.angularVelocity.rotate( r, result );
  ...
  integrate(dt) {
    ...
    const angularVelo = this.angularVelocity; //console.log( "new velocity (was):", this.angularVelocity );
    ...
    angularVelo.z += dt * (e[6] * tx + e[7] * ty + e[8] * tz); //console.log( "new velocity (is):", this.angularVelocity );
...
const Body_applyForce_rotForce = new Vec3i();
new Vec3();
const Body_applyLocalForce_worldForce = new Vec3();
const Body_applyLocalForce_relativePointWorld = new Vec3();
const Body_applyImpulse_velo = new Vec3();
const Body_applyImpulse_rotVelo = new Vec3i();
...
class ContactEquation extends Equation {
  ...
  computeB(h) {
    ...
    ri.cross(n, rixn, true);
    rj.cross(n, rjxn, true); // g = xj+rj -(xi+ri)
    // G = [ -ni  -rixn  ni  rjxn ]
...
class RaycastVehicle {
  ...
  updateVehicle(timeStep) {
    ...
    for (let i = 0; i < numWheels; i++) {
      ...
      chassisBody.applyImpulse(chassisBody.mass, impulse, relpos);
    }
  ...
  updateWheelTransform(wheelIndex) {
    ...
    rotatingOrn.mult(steeringOrn, q);
    q.mult(this.chassisBody.quaternion, q); //this.chassisBody.quaternion.mult(steeringOrn, q)
  ...
  updateFriction(timeStep) {
    ...
    for (let i = 0; i < numWheels; i++) {
      ...
      if (groundObject) {
        ...
        axlei.normalize();
        surfNormalWS.cross(axlei, forwardWS[i], true);
    ...
    for (let i = 0; i < numWheels; i++) {
      ...
      if (wheel.forwardImpulse !== 0) {
        ...
        chassisBody.applyImpulse(chassisBody.mass, impulse, rel_pos);
      }
      ...
      if (wheel.sideImpulse !== 0) {
        ...
        chassisBody.applyImpulse(chassisBody.mass, sideImp, rel_pos); //apply friction impulse on the ground

        sideImp.scale(-1, sideImp);
        groundObject.applyImpulse(groundObject.mass, sideImp, rel_pos2);
      }
    }
  }
    ...
    if (Neq !== 0) {
      ...
      for (iter = 0; iter !== maxIter; iter++) {
        ...
        for (let j = 0; j !== Neq; j++) {
          ...
          deltalambda = invC * (B - GWlambda - c.eps * lambdaj); //console.log( "lambda was now: wlambda",deltalambda, watch );
          ...
          c.addToWlambda(deltalambda); //console.log( "lambda is now: wlambda", watch );
      ...
      for (let i = 0; i !== Nbodies; i++) {
        const b = bodies[i];
        const v = b.velocity;
        const w = b.angularVelocity; //console.log( "lambda velocity (was):", b.angularVelocity, b.wlambda );

        b.vlambda.vmul(b.linearFactor, b.vlambda);
        v.vadd(b.vlambda, v);
        b.wlambda.vmul(b.angularFactor, b.wlambda);
        w.vadd(b.wlambda, w); //console.log( "lambda velocity (is):", b.angularVelocity );
...
class World extends EventTarget {
  ...
  internalStep(dt) {
    ...
    this.stepnumber; //const quatNormalize = stepnumber % (this.quatNormalizeSkip + 1) === 0
