	...
	function drawGrid(normalVertices,normalColors, curSliders) {
		...
		if( mapPolar ) {
			...
			for( let r = 0.01; r <= (range); r += (step/ ( 1.3*range/(Math.PI))), rline++ ){
				...
				// gamme pi/2 +/-e  a little bit is a bunch of geodesics that do not turn around the sphere.

				// Math.PI/2+0.1 -curSliders.lnQY[0]
				// - this is the same set of extreme outer geodesics...
				// Math.PI +/-0.1 - sliders
				...
				for( let gamma = -Math.abs(curSliders.lnQX[0])-0.1 +curSliders.lnQY[0]; gamma <= Math.abs(curSliders.lnQX[0])+0.1 +curSliders.lnQY[0]; gamma += Math.PI/20, gamline++ ){
					...
					if( weylCurvature )
						...
						//     to 4pi/2 the north pole again, but not in the same phase.  (This is the opposite polar phase though on the graph)
						//
					        //  lng = 0 initial to 1pi - then north pole is in south pole phase
						//    (this should probably be divided by 2, so 2pi lng is 1 full rotation)
						//     forward/right basis rotates 360 degrees in 1 pi rotations of this.
                                                //        1pi to 2pi      - completes 1 cycle. 
						//     forward/right basis rotates another 360 degrees in 1 pi rotations of this.
						//
						//  gamma = 0 is initial , this is the longitude step on the sphere.
						//        2pi is initial again (1 cycle)
						// gamma and lng sort of work together in the same direction... but gamma is just a sub step on the general orienetation of lng.
					        
						//let ang = Math.acos( -Math.sin(gamma+lng)*Math.sin(r/2) )*2 ;
						//const Cx = Math.sin(lng) * Math.cos(r/2);
						//const Cy = Math.sin(r/2) * Math.cos(lng + gamma);
						//const Cz = Math.cos(lng) * Math.cos(r/2);
...
if(0) {
				...
				if( showNormalProjection ) {
					...
					const basis = lnQ.update().getBasis();
					if( useForward ) {
					tmpPoint.x = basis.forward.x * spaceScale*1.43 ;
					tmpPoint.y = basis.forward.y * spaceScale*1.43 ;
					tmpPoint.z = basis.forward.z * spaceScale*1.43 ;
					} else if( useRight ) {
					tmpPoint.x = basis.right.x * spaceScale*1.43 ;
					tmpPoint.y = basis.right.y * spaceScale*1.43 ;
					tmpPoint.z = basis.right.z * spaceScale*1.43 ;
					} else{
					tmpPoint.x = basis.up.x * spaceScale*1.43 ;
					tmpPoint.y = basis.up.y * spaceScale*1.43 ;
					tmpPoint.z = basis.up.z * spaceScale*1.43 ;
					...
					if( showGrid && draw ) {
						...
						if( r > wraps ) {
							normalVertices.push( new THREE.Vector3( (oldp.x)*spaceScale ,(oldp.y)*spaceScale    , (oldp.z)*spaceScale ))
							if(useForward )
								normalVertices.push( new THREE.Vector3( (basis.forward.x)*spaceScale ,(basis.forward.y)*spaceScale    , (basis.forward.z)*spaceScale ))
							else if(useRight )
								normalVertices.push( new THREE.Vector3( (basis.right.x)*spaceScale ,(basis.right.y)*spaceScale    , (basis.right.z)*spaceScale ))
							else
								normalVertices.push( new THREE.Vector3( (basis.up.x)*spaceScale ,(basis.up.y)*spaceScale    , (basis.up.z)*spaceScale ))
						...
						if(useForward ){
							oldp.x = basis.forward.x;
							oldp.y = basis.forward.y;
							oldp.z = basis.forward.z;
						}else	if(useRight ){
							oldp.x = basis.right.x;
							oldp.y = basis.right.y;
							oldp.z = basis.right.z;
						}else{
							oldp.x = basis.up.x;
							oldp.y = basis.up.y;
							oldp.z = basis.up.z;
