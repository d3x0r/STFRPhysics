...
lnQuat.prototype.applyDel = function( v, del ) {
	if( v instanceof lnQuat ) {
		const result = new lnQuat(
			function() {
				const q = v;
				const as = Math.sin( q.nL/2 * del );
				const ac = Math.cos( q.nL/2 * del );
				const ax = q.nx;
				const ay = q.ny;
				const az = q.nz;
	...
	// 3+2 +sqrt+exp+sin
		...
		const s  = Math.sin( (q.nL/2)*del );//q.s;
		const nst = s/q.nR; // sin(theta)/r    normal * sin_theta
		const qw = Math.cos( (q.nL/2)*del );  // quaternion q.w  = (exp(lnQ)) [ *exp(lnQ.W=0) ]
