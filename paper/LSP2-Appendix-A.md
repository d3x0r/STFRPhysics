...
Can also be written as (converting magnitude to square root of squared length):
...
Multiply right-side top and bottom by $\frac{\sqrt{\left( C^{2} - {\vec{V}}^{2} \right)}}{\sqrt{\left( C^{2} - {\vec{V}}^{2} \right)}}$
