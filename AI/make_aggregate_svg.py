...
def make_histogram_svg():
    ...
    svg.append(f'<text x="{margin["l"]+pw/2}" y="{height-12}" text-anchor="middle" class="lbl">relative RMS (RMS / &lt;V_obs&gt;)</text>')
    ...
    bw = pw / 30
    for b in range(30):
        # Low-amp (good fits) on bottom
        h_low = (bins_low[b] / (max_count * 1.10)) * ph
        h_high = (bins_high[b] / (max_count * 1.10)) * ph
        y0 = margin['t'] + ph - h_low
        x0 = margin['l'] + b * bw
        if bins_low[b] > 0:
            svg.append(f'<rect x="{x0+1}" y="{y0}" width="{bw-2}" height="{h_low}" class="bar-low"/>')
        if bins_high[b] > 0:
            svg.append(f'<rect x="{x0+1}" y="{y0-h_high}" width="{bw-2}" height="{h_high}" class="bar-high"/>')
    ...
    p5  = sum(1 for s in stats if s['rms_rel'] < 0.05)
    p10 = sum(1 for s in stats if s['rms_rel'] < 0.10)
    p20 = sum(1 for s in stats if s['rms_rel'] < 0.20)
    svg.append(f'<text x="{nx}" y="{ny+28}" class="lbl">&lt;5%:  {p5}/{len(stats)} ({100*p5/len(stats):.0f}%)</text>')
    svg.append(f'<text x="{nx}" y="{ny+44}" class="lbl">&lt;10%: {p10}/{len(stats)} ({100*p10/len(stats):.0f}%)</text>')
    svg.append(f'<text x="{nx}" y="{ny+60}" class="lbl">&lt;20%: {p20}/{len(stats)} ({100*p20/len(stats):.0f}%)</text>')
