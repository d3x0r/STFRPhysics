...
The audited mass estimates are tabulated in the generated aggregate CSV under `AI/galaxy_svgs` as `m_total_msun` and `log10_m_total_msun`, using the same enforced profile and conversion formula. In the current aggregate output, the fitted total masses span $8.3 \times 10^7\,M_\odot$ to $5.3 \times 10^{12}\,M_\odot$, with median $3.0 \times 10^{10}\,M_\odot$. In logarithmic form, this corresponds to $\log_{10}(M/M_\odot)$ from 7.92 to 12.72, with median 10.48.

The same audited mass estimates can be compared against the SPARC-derived baryonic mass inside the outermost measured rotation point, computed from the published gas, disk, and bulge velocity components with the fixed global mass-to-light ratios used for the MOND/RAR reference. This comparison is shown in Figure 4.

<p align="center">
  <img src="../../galaxy_svgs/_aggregate_mass_comparison.svg" alt="Fitted support mass compared with SPARC-derived baryonic mass">
</p>

**Figure 4.** *Fitted support mass compared with SPARC-derived baryonic mass inside the outermost measured rotation point for the 175 SPARC-format galaxies. The dashed diagonal is the one-to-one mass line; point color follows gas fraction, from compact-dominated systems in red to gas-dominated systems in blue. File: `AI/galaxy_svgs/_aggregate_mass_comparison.svg`.*

The comparison shows two features that should be kept separate. First, the points are strongly correlated across the sample: a log-log fit to the plotted masses has slope $\simeq 1.04$ with Pearson correlation $r \simeq 0.963$ ($R^2 \simeq 0.927$). Thus the fitted support mass tracks the galaxy-to-galaxy scale of the SPARC baryonic estimate closely, rather than behaving as an unrelated normalization assigned independently to each galaxy. Second, the normalization is not one-to-one. The median fitted-support to SPARC-baryonic mass ratio is approximately 5.04, with the central 68% of the sample spanning roughly 2.6--8.8. In the present calibration this is best interpreted as a mass-scale normalization discrepancy between the framework's fitted support mass and the SPARC photometric baryonic mass, not as evidence for a halo-like reservoir: the fitted support remains geometrically bounded by the measured galaxy as shown in §5.2. Resolving whether the factor arises from the Newtonian-baseline calibration, the mapping between occupancy and physical surface density, the use of SPARC decomposition masses, or a genuine framework-specific support normalization requires a separate mass-calibration analysis.
...
**Figure 5.** *Aggregate scatter of framework σ-RMS against MOND/RAR σ-RMS per galaxy. Points below the diagonal are galaxies where the framework produces the smaller error-normalized residual. File: [AI/Revised/fig/galaxy-fits/fits/_aggregate_framework_vs_mond.svg](https://github.com/d3x0r/STFRPhysics/tree/master/AI/Revised/fig/galaxy-fits/fits/_aggregate_framework_vs_mond.svg/).*
...
**The relationship between the framework's recovered mass and external mass estimates is only partially characterized here.** Section 5.3 compares the fitted support masses against SPARC-derived baryonic masses and finds a strong near-linear log-log correlation, but also a median normalization offset of about a factor of five. The framework's amplitude-to-mass conversion is calibrated against the Milky Way's known mass, and the recovered masses for SPARC galaxies fall within physically reasonable ranges and span the expected scale. However, interpreting the normalization offset relative to (a) photometric baryonic mass estimates from SPARC and (b) full dynamical-mass estimates that include a halo contribution is a separate analysis that requires the framework's gravitational response to be developed quantitatively beyond the level of an empirical fit. That analysis is deferred to follow-up work. What this paper establishes about mass most directly is the *geometric* claim — no mass past the visible extent — while the absolute mass scale remains a calibration question.
...
This paper is one of a series developing the displacement framework as an alternative to general relativity in the gravitational sector and to special relativity's primitive Lorentz structure in the kinematic sector. The framework's overall postulate structure, derivational order, and ontological commitments are developed in [[2](https://doi.org/10.5281/zenodo.20472270), [3](https://doi.org/10.5281/zenodo.20075441), [6](https://doi.org/10.5281/zenodo.19702909)]. The weak-field observational tests of the framework — Mercury perihelion advance, Shapiro delay, light deflection, Gravity Probe B, gravitational radiation — are presented in [[4](https://doi.org/10.5281/zenodo.20075467)]; the comparison with general relativity at the weak-field and strong-field observational levels is discussed in [[5](https://doi.org/10.5281/zenodo.20159472)]. The framework's philosophical foundations and the open frontier are taken up in [[6](https://doi.org/10.5281/zenodo.19702909)].
...
7. The Python script `AI/make_aggregate_comparison.py` reads the raw preset and SPARC-format data under `AI` and generates the aggregate framework / MOND / Newtonian comparison outputs under `AI/galaxy_svgs`, including `_aggregate_model_comparison.csv`, `_aggregate_model_comparison.json`, `_aggregate_edge_bounded.svg`, `_aggregate_framework_vs_mond.svg`, and `_aggregate_mass_comparison.svg`.
