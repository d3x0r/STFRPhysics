...
export function generateGalaxySvg(name, data, opts = {}) {
  ...
  if (!P) throw new Error(`No preset for galaxy: ${name}`);
  ...
  // Geometry
  const margin = { l: 70, r: 85, t: 55, b: 55 };
  ...
  lines.push('  .sig { stroke: #b03030; stroke-width: 1.6; fill: none; stroke-dasharray: 4,2; opacity: 0.85; }');
  ...
  // Title / subtitle
  lines.push(`<text x="${width/2}" y="22" text-anchor="middle" class="title">${esc(name)}</text>`);
  const sub = `rMax=${rMax.toFixed(1)} kpc, N=${results.length}, RMS=${rms.toFixed(1)} km/s (${(relRms*100).toFixed(1)}%), amp=${P.amp.toFixed(0)}`;
  lines.push(`<text x="${width/2}" y="38" text-anchor="middle" class="subtitle">${esc(sub)}</text>`);
  ...
  lines.push(`<text x="${rlx+30}" y="${rly+22}" class="lbl-r">Σ(r) smoothed</text>`);
  if (rhoPeakMsun != null) {
