...
// --- Baryonic and MOND/RAR baselines ---

function signedSquare(v) {
  // SPARC component curves use signed gas terms in some galaxies.
  // The contribution to V_bar^2 should preserve that sign convention.
  return v * Math.abs(v);
}
...
function mondVelocity(row, opts = {}) {
  // Simple McGaugh/Lelli/Schombert RAR-style interpolating form:
  //   g_obs = g_bar / (1 - exp(-sqrt(g_bar / g_dagger)))
  // with g_dagger expressed internally in (km/s)^2/kpc.
...
function computeMetrics(results, field, k = 0) {
  let sum2 = 0;
  let sig2 = 0;
  let rel2 = 0;
  let maxSig = 0;
...
export function generateGalaxySvg(name, data, opts = {}) {
  ...
  const results = rows.map(row => {
    ...
    return { r: row.r, x, v_obs: row.vObs, err_v: row.errV, v_fw, v_n, v_mond };
  });

  // Stats. The framework k value is deliberately configurable because the exposed
  // sliders are role-constrained rather than ordinary unconstrained fit parameters.
  const fwK = opts.frameworkK ?? opts.kFramework ?? 0;
  const fwStats = computeMetrics(results, 'v_fw', fwK);
  const ntStats = computeMetrics(results, 'v_n', opts.newtonK ?? 1);
  const mondStats = computeMetrics(results, 'v_mond', opts.mondK ?? 0);
  const rms = fwStats.rms;
  const relRms = fwStats.relRms;
  ...
  const sub = `rMax=${rMax.toFixed(1)} kpc, N=${results.length}, FW=${fmtMetric(fwStats)}, amp=${P.amp.toFixed(0)}`;
  ...
  // MOND/RAR baseline from SPARC component curves
  const mondPts = results
    .filter(r => r.v_mond != null && Number.isFinite(r.v_mond))
    .map(r => [xPx(r.r), yV(r.v_mond)]);
  if (mondPts.length > 1) {
    lines.push(`<path d="${pathFromPoints(mondPts)}" class="mond"/>`);
  }
  ...
  for (let i = 0; i <= Nstops; i++) {
    ...
    const bright = Math.max(0, Math.min(255, Math.round(brightNorm * 255)));
	 if( P.newtonAmp < 50 )  stops.push(`<stop offset="${frac.toFixed(4)}" stop-color="rgb(${bright},${bright/4},${bright/4})"/>`);
...
export { rho, buildFieldCache, shapeFW, shapeN, mEnc, interp, mondVelocity, vBarSquared, computeMetrics };
