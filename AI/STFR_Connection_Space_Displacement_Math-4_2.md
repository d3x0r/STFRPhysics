...
### Frame Dragging: Open Mechanism

The gravitomagnetic drag velocity at orbital radius from Earth's rotation is $v_{gm} = GJ_\oplus/(C^2 r^2) \approx 3.5 \times 10^{-16} \cdot C$ — negligibly small. A simple $W_{STFR}$ calculation using the bulk rotational velocity of the displacement field does not reproduce the 39.2 mas/yr frame-dragging signal at the right scale.

The physical mechanism for frame dragging in STFR must come from the *structural asymmetry* of the rotating displacement field — the toroidal component of $D(r,d)$ introduced by Earth's spin — rather than from a bulk drag velocity. The rotating Earth's displacement field is not perfectly spherically symmetric; its equatorial bulge and the dragging of displacement isocontours by the spin create an asymmetric $\Sigma(r, \theta)$ that a polar-orbiting gyroscope samples differently at different orbital phases. This is the STFR analog of the Lense-Thirring gravitomagnetic field, but arising from the displacement geometry rather than from a metric perturbation.

The tidal curvature of the displacement field across the gyroscope body (gradient $dD/dr \approx -d^2/r^3$) provides a second candidate mechanism — differential $F \cdot H$ on the near-Earth vs. far-Earth sides of the gyroscope — that is also sensitive to the asymmetry introduced by Earth's rotation.

**Frame dragging remains an open calculation** requiring the full rotating displacement field solution.

### Assessment

The geodetic result is well-explained by the same three-mechanism structure that accounts for Mercury precession. Frame dragging is the remaining open problem requiring development of the rotating displacement field geometry.
...
4. **GPB frame dragging:** Geodetic precession is accounted for by the three-contribution structure ($W_{STFR}$ + time dilation + $\Sigma(r)$ gradient, giving 6.59 arcsec/yr). Frame dragging (39.2 mas/yr) remains open — requires the full rotating displacement field solution, including the toroidal asymmetry of $D(r,\theta)$ from Earth's spin and the tidal curvature mechanism.
